...
# Combine multiple Subcontractor types into a single value
...
# Create groups for Rate using the cut function
breaks <- c(50, 100, 125, 130, 135, 140, 145, 150, 175, 200)
RateGroups <- cut(train$Rate, breaks= breaks)
summary(RateGroups)
train$OrigRateGroup <- RateGroups

# Create groups for Duration using cut function
durationBreaks <- c(0, 90, 180, 270, 360, 450)
DurationGroups <- cut(as.numeric(train$OrigDuration), breaks = durationBreaks, right = FALSE)
summary(DurationGroups)
train$OrigDurationGroup <- DurationGroups
